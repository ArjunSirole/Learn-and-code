...
// Set up automatic retries for axios requests
...
function buildTumblrApiUrl(blogName: string, start: number, end: number): string {
...
function parseRangeInput(range: string): [number | null, number | null] {
  ...
  return [isNaN(startIndex) ? null : startIndex, isNaN(endIndex) ? null : endIndex];
}

function isValidRange(startIndex: number | null, endIndex: number | null): boolean {
  return startIndex !== null && endIndex !== null && startIndex >= 1 && endIndex >= startIndex;
}
...
async function fetchTumblrPostsAndDisplayImages() {
  ...
  if (!isValidRange(startIndex, endIndex)) {
    console.log('Invalid range input. Please ensure the range is in the format "start-end" where start <= end.');
    return;
  }
